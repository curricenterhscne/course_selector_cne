...
- 사용자 확인 필요 사항: 없음 (커밋 936468f으로 완료)

---

## [2026-05-20] 데이터 갱신 파이프라인 구축

- 신규 파일: update_data.py
- 핵심 기능:
  1. `--dry-run`: 변경사항 미리보기 (파일 쓰기 없음)
  2. `--year YYYY`: 특정 연도만 처리 (생략 시 KNOWN_YEARS 전체)
  3. 변경 감지: 신규/변경/삭제/동일 학교 분류 출력
  4. 데이터 품질 검증: 이름 공백·학점 정보 누락 자동 감지
  5. 신규 연도 추가 시 index.html year-pill 버튼 자동 패치
  6. 처리 후 git add/commit 명령어 안내 출력
- 검증 결과:
  - dry-run 전체: 2025(93개 동일), 2026(N100000214 변경 감지 + 품질 경고) ✓
  - dry-run --year 2026 단일 모드 정상 ✓
- CLAUDE.md에 "데이터 갱신 방법" 섹션 추가
- 사용자 확인 필요 사항: 없음

---

## [2026-05-20] 브라우저 기반 데이터 관리 도구 (admin.html)

- 신규 파일: admin.html
- 대상: 코딩을 모르는 담당자
- 핵심 기능:
  1. xlsx 드래그앤드롭 (복수 파일 동시 처리)
  2. SheetJS로 Data/Schools/CourseDB 시트 파싱
  3. 로컬 서버 실행 시 기존 JSON과 diff (신규/변경/삭제/동일 분류)
  4. 데이터 품질 검증 (과목명 공백 등) 자동 표시
  5. JSZip으로 전체 data/ 구조 ZIP 다운로드
  6. 신규 연도 감지 시 주의문 표시
  7. 다운로드 후 단계별 안내 (GitHub Desktop 사용법)
